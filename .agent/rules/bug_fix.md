...
# 核心工具：Browser MCP (浏览器自动化诊断服务)
你现在拥有了一套强大的新工具：**Browser MCP**。它允许你直接自动化控制浏览器，让你能像真实用户一样与网页交互来诊断问题。你应该在以下场景中利用这套工具：
*   **自动化复现Bug**: 设计一个操作序列（导航、点击、输入）来稳定地复现问题。
*   **检查隐藏的错误**: 使用 `browser_get_console_logs` 来查看开发者控制台中是否有JavaScript错误或网络请求失败。
*   **视觉验证**: 使用 `browser_screenshot` 来获取视觉证据，确认UI变更或异常状态。
*   **交互式诊断**: 组合使用工具，系统性地测试应用的每一个部分，定位到出问题的那个具体交互。

### 工具列表
*   `browser_navigate`: 导航到指定URL。 (参数: `url`)
*   `browser_go_back`: 浏览器后退。
*   `browser_go_forward`: 浏览器前进。
*   `browser_snapshot`: **(关键工具)** 获取当前页面的可访问性快照，返回页面上所有可交互元素的描述和引用(`ref`)。**这是与任何元素交互前的必需步骤。**
*   `browser_screenshot`: 截取当前页面的完整屏幕截图。
*   `browser_get_console_logs`: 获取浏览器开发者工具中的控制台日志。
*   `browser_click`: 点击一个元素。 (参数: `element`, `ref`)
*   `browser_hover`: 鼠标悬停在一个元素上。 (参数: `element`, `ref`)
*   `browser_type`: 在输入框中输入文本。 (参数: `element`, `ref`, `text`, `submit`)
*   `browser_select_option`: 从下拉菜单中选择一个选项。 (参数: `element`, `ref`, `values`)
*   `browser_press_key`: 模拟键盘按键。 (参数: `key`)
*   `browser_wait`: 等待固定时间，用于等待页面加载。 (参数: `time`)
...
1.  **规划 (Plan & Document)**
    ...
    *   c. **初步假设**: 其次，基于Bug描述，提出对问题根源的初步假设。
    *   d. **诊断策略与循环**:
        *   **方法**: 首选通过 **Browser MCP/playwright 服务，获取控制台日志或进行动态自动化测试**，其次再进行静态代码分析。
        *   **计划内容**: 计划必须包含【自动化诊断】步骤，明确说明要执行的【测试操作】和要收集的【证据】(如获取控制台日志、截图)。
        *   **迭代**: 我将不断迭代（**计划 -> 测试 -> 分析证据 -> 修正假设**），直到锁定根源。
    *   e. **文件化**: 我会将此清单保存在`docs/`目录下，文件名格式为 `[任务简述]-bugfix-[YYYYMMDD-HHMMSS].md`。
