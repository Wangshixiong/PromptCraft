 ...
 * 职责: 管理原生JavaScript标签组件的初始化、数据绑定和事件处理
 * 创建时间: 2025-01-02
 */
...
class TagComponentManager {
    ...
    createTagComponent() {
        ...
        input.type = 'text';
        input.placeholder = this.currentTags.length === 0 ? chrome.i18n.getMessage('tagInputPlaceholderEmpty') : chrome.i18n.getMessage('tagInputPlaceholder');
    ...
    renderRecommendedTags() {
        ...
        if (availableRecommendedTags.length === 0) {
            ...
            emptyMessage.textContent = chrome.i18n.getMessage('noRecommendedTags');
